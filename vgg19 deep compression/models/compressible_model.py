...
class Compressible(object):
    ...
    def initialize_variables(self,
        ...
        n_blocks = 1 + int((num_vars - 1) / block_size)
        shape = [int(n_blocks), int(block_size)]
        ...
        p = tf.compat.v1.distributions.Normal(loc=0., scale=self.p_scale)
        mu_init_list = []
        for (type, val), size in zip(initializers, var_sizes):
            if type == 'normal':
                mu_init_list.append(np.random.normal(size=int(size), loc=0., scale=val))
            elif type == 'uni':
                mu_init_list.append(np.random.uniform(-val, val, size=int(size)))
            elif type == 'zero':
                mu_init_list.append(np.zeros(int(size)))
            else:
                assert False
        ...
        init_inv_permuted = np.concatenate((mu_init,
                                            np.zeros(int(num_vars_ub - num_vars))),
        ...
        self.w_dist = tf.compat.v1.distributions.Normal(loc=mu, scale=sigma)
        ...
        block_kl = tf.reduce_sum(tf.compat.v1.distributions.kl_divergence(self.w_dist, p), axis=1)
        ...
        split_weights = tf.split(permuted_weights, var_sizes + [int(num_vars_ub - num_vars)])
